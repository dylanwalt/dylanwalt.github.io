...
Right ≈ 50%: a calm OneView control surface (live ring, four framework
rows, evidence log, and coverage line) — not a radar, not a terminal.
...
LoopNarrative, OneViewBand, RegionalPresence, FrameworksRail, InsightsFeed,
LinkedInPostsSection, PressSection, ContactStage, SiteFooter). Each section
...
- **Staged entrance reveal** on every section (translate-y 24px → 0, opacity
  0 → 1, stagger 80ms).
- **Scroll-pinned Control loop** — three states (`Observe → Remediate → Verify`)
  ...
  evidence rows swap.
- **Counter animation** (count-up) on the proof rail / OneView framework count
  on first view.
- **Live ring** animates `stroke-dashoffset` from open to near-full on
  intersection.
...
- Primary `Request a demo` lives in: global nav (right), sticky subnav,
  hero, end of every product band, and the contact stage.
- Secondary `See OneView` (anchor to the OneView band) lives in the hero and
  the sticky subnav.
- Tertiary `Open the form` lives in the closing contact stage.
...
- `useReducedMotion()` short-circuits the Control loop into a static three-up
  grid; the diagram renders at its final state.
- The live ring stays static at its final offset.
...
- Focus styles: 2px brand-green ring with 2px offset on every interactive
  element.
...
V5 inherits the V4 architecture and sharpens the hero, regional band, and
CTA stage:
...
- **Logo**: new shield + padlock + circuit-trace SVGs (`logo-shield-white.svg`
  and `logo-shield-color.svg`) in `public/port443-v5/`.

## V5 iterative refinements (post-review)

User feedback on V5 applied in-place rather than spinning a new version:
