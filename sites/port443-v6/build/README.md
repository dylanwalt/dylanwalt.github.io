...
Sixth pass at the Port443 rebuild, produced with the
`website-rebuild-pro` skill.

V6 promotes the richer landing exploration to the homepage and
keeps the site pointed toward a sharper, more immersive control-room feel:
...
```bash
NEXT_PUBLIC_BASE_PATH=/port443-v6 npm run build
```
...
- Public assets live at `public/port443-v6/`. The site now uses
  `logo-full.webp` and `logo-mark.webp` from that folder for shared brand
  placements, and still resolves them as `/port443-v6/<file>` so `basePath`
